...
class GraphStoreService {
  ...
  public loadOntology(ontologyContent: string, filename: string): void {
    const quads: Quad[] = []
    const prefixes: { [key: string]: NamedNode<string> } = {}
    this.parser.parse(ontologyContent, {
      onQuad: (err, quad) => {
        quads.push(quad)
      },
      onPrefix: (prefix, ns) => {
        prefixes[prefix] = ns as NamedNode<string>
      }
    })
    const ontologyQuad = quads.find(
      (quad) =>
        quad.predicate.value === 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type' &&
        quad.object.value === 'http://www.w3.org/2002/07/owl#Ontology'
    )
    const ontologyNamedNode = ontologyQuad ? ontologyQuad.subject : namedNode(filename)
    this.store.addQuads(
      quads.map((quad) => {
        return new Quad(quad.subject, quad.predicate, quad.object, ontologyNamedNode)
      })
    )
  }
  ...
  public getLabel(uri: string): string {
    return (
      this.getObjectValue(uri, 'http://www.w3.org/2000/01/rdf-schema#label') ||
      this.getObjectValue(uri, 'http://www.w3.org/2004/02/skos/core#prefLabel') ||
      uri.split('/').pop()?.split('#').pop() ||
      uri
    )
  }
  public getComment(uri: string): string {
    return (
      this.getObjectValue(uri, 'http://www.w3.org/2000/01/rdf-schema#comment') ||
      this.getObjectValue(uri, 'http://www.w3.org/2004/02/skos/core#definition') ||
      uri.split('/').pop()?.split('#').pop() ||
      uri
    )
  }
  ...
  public getSubjectAnnotationQuads(uri: string): Quad[] {
    ...
    return this.store.getQuads(namedNode(uri), null, null, null)
    ...
    const quad =
      quads.find(
        (q) =>
          ...
          (q.object as Literal).language === language
      ) || quads[0]
